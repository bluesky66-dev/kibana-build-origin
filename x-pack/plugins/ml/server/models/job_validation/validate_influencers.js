...
async function validateInfluencers(job) {
  ...
  if (influencers.length === 0 && job.analysis_config.detectors.length === 1 && detectorFieldNames.length === 0) {
    return Promise.resolve([]);
  }

  if (influencers.length <= INFLUENCER_LOW_THRESHOLD && detectorFieldNames.length >= DETECTOR_FIELD_NAMES_THRESHOLD) {
