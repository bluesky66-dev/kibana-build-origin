...
exports.mergeExceptionLists = exports.filterInstalledRules = exports.getRulesToUpdate = void 0;
/*
...
const getRulesToUpdate = (rulesFromFileSystem, installedRules) => {
  return rulesFromFileSystem.filter(ruleFromFileSystem => filterInstalledRules(ruleFromFileSystem, installedRules)).map(ruleFromFileSystem => mergeExceptionLists(ruleFromFileSystem, installedRules));
};
/**
...
const filterInstalledRules = (ruleFromFileSystem, installedRules) => {
  return installedRules.some(installedRule => {
    return ruleFromFileSystem.rule_id === installedRule.params.ruleId && ruleFromFileSystem.version > installedRule.params.version;
  });
};
/**
...
const mergeExceptionLists = (ruleFromFileSystem, installedRules) => {
  if (ruleFromFileSystem.exceptions_list != null) {
    const installedRule = installedRules.find(ruleToFind => ruleToFind.params.ruleId === ruleFromFileSystem.rule_id);
    ...
    if (installedRule != null && installedRule.params.exceptionsList != null) {
      ...
      const fileSystemExceptions = ruleFromFileSystem.exceptions_list.filter(potentialDuplicate => installedExceptionList.every(item => item.list_id !== potentialDuplicate.list_id));
