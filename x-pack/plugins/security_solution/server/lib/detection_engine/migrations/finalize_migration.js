...
var _get_index_count = require("../index/get_index_count");
...
const finalizeMigration = async ({
  ...
  const sourceCount = await (0, _get_index_count.getIndexCount)({
    esClient,
    index: sourceIndex
  });
  const destinationCount = await (0, _get_index_count.getIndexCount)({
    esClient,
