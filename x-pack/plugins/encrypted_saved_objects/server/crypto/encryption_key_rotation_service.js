...
class EncryptionKeyRotationService {
  ...
  async rotate(request, {
    ...
    const typeRegistry = savedObjects.getTypeRegistry(); // We need to retrieve all SavedObject types which have encrypted attributes, specifically
    ...
    this.options.logger.info(`Saved Objects with the following types [${registeredSavedObjectTypes}] will be processed.`); // We need two separate Saved Objects clients for the retrieval and update. For retrieval we
    ...
    const processedObjectIDs = new Set(); // Until we get scroll/search_after support in Saved Objects client we have to retrieve as much objects as allowed
    ...
    while (true) {
      ...
      if (batch === 0) {
        ...
        result.total = savedObjectsToDecrypt.total; // Since we process live data there is a theoretical chance that we may be getting new
  ...
  async getSavedObjectsToReEncrypt(savedObjects, processedObjectIDs, typeRegistry, user) {
    ...
    for (const savedObject of savedObjects) {
      var _savedObject$namespac; // We shouldn't process objects that we already processed during previous iterations.
      ...
      decryptedSavedObjects.push({ ...savedObject,
