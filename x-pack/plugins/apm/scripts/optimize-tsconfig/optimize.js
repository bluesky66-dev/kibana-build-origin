...
function prepareParentTsConfigs() {
  return Promise.all([path.resolve(xpackRoot, 'tsconfig.json'), path.resolve(kibanaRoot, 'tsconfig.base.json'), path.resolve(kibanaRoot, 'tsconfig.json')].map(async filename => {
    const config = json5.parse(await readFile(filename, 'utf-8'));
    await writeFile(filename, JSON.stringify({ ...omit(config, 'references'),
...
async function addApmFilesToXpackTsConfig() {
  ...
  await writeFile(xpackTsConfig, JSON.stringify({ ...config,
    ...template
  }, null, 2), {
    encoding: 'utf-8'
  });
