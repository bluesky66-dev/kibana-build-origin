...
function estimateBucketSpanFactory(client) {
  ...
  class BucketSpanEstimator {
    constructor({
      ...
      this.polledDataChecker = new PolledDataChecker(this.index, this.timeField, this.duration, this.query);
      ...
      if (this.aggTypes.length === this.fields.length) {
        ...
        for (let i = 0; i < this.aggTypes.length; i++) {
          if (this.splitField === undefined) {
            ...
            this.checkers.push({
              check: new SingleSeriesChecker(this.index, this.timeField, this.aggTypes[i], this.fields[i], this.duration, this.query, this.thresholds, this.runtimeMappings),
              result: null
            });
          } else {
            ...
            for (let j = 0; j < this.splitFieldValues.length; j++) {
              ...
              this.checkers.push({
                check: new SingleSeriesChecker(this.index, this.timeField, this.aggTypes[i], this.fields[i], this.duration, queryCopy, this.thresholds, this.runtimeMappings),
                result: null
              });
            }
          }
  ...
  const getRandomFieldValues = function (index, field, query, runtimeMappings) {
    ...
    return new Promise((resolve, reject) => {
      ...
      getFieldCardinality(index, field).then(value => {
    ...
    return new Promise((resolve, reject) => {
      ...
      asInternalUser.cluster.getSettings({
        ...
        if (formConfig.splitField !== undefined) {
          getRandomFieldValues(formConfig.index, formConfig.splitField, formConfig.query, formConfig.runtimeMappings).then(splitFieldValues => {
