  ...
  return next => doc => {
    if (series.metrics.some(filter)) {
      series.metrics.filter(filter).forEach(metric => {
        (0, _helpers.overwrite)(doc, `aggs.${series.id}.aggs.timeseries.aggs.${metric.id}-numerator.filter`, _server.esQuery.buildEsQuery(indexPatternObject, metric.numerator, [], esQueryConfig));
        (0, _helpers.overwrite)(doc, `aggs.${series.id}.aggs.timeseries.aggs.${metric.id}-denominator.filter`, _server.esQuery.buildEsQuery(indexPatternObject, metric.denominator, [], esQueryConfig));
