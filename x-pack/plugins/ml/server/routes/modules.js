...
function recognize(client, mlClient, savedObjectsClient, jobSavedObjectService, request, indexPatternTitle) {
  const dr = new _data_recognizer.DataRecognizer(client, mlClient, savedObjectsClient, jobSavedObjectService, request);
  return dr.findMatches(indexPatternTitle);
}

function getModule(client, mlClient, savedObjectsClient, jobSavedObjectService, request, moduleId) {
  const dr = new _data_recognizer.DataRecognizer(client, mlClient, savedObjectsClient, jobSavedObjectService, request);
...
function setup(client, mlClient, savedObjectsClient, jobSavedObjectService, request, moduleId, prefix, groups, indexPatternName, query, useDedicatedIndex, startDatafeed, start, end, jobOverrides, datafeedOverrides, estimateModelMemory, applyToAllSpaces) {
  const dr = new _data_recognizer.DataRecognizer(client, mlClient, savedObjectsClient, jobSavedObjectService, request);
...
function dataRecognizerJobsExist(client, mlClient, savedObjectsClient, jobSavedObjectService, request, moduleId) {
  const dr = new _data_recognizer.DataRecognizer(client, mlClient, savedObjectsClient, jobSavedObjectService, request);
...
function dataRecognizer({
  ...
  router.get({
    ...
    try {
      ...
      const results = await recognize(client, mlClient, context.core.savedObjects.client, jobSavedObjectService, request, indexPatternTitle);
      return response.ok({
        body: results
      });
    } catch (e) {
  ...
  router.get({
    ...
    try {
      ...
      const results = await getModule(client, mlClient, context.core.savedObjects.client, jobSavedObjectService, request, moduleId);
      return response.ok({
        body: results
      });
    } catch (e) {
  ...
  router.post({
    ...
    try {
      ...
      const result = await setup(client, mlClient, context.core.savedObjects.client, jobSavedObjectService, request, moduleId, prefix, groups, indexPatternName, query, useDedicatedIndex, startDatafeed, start, end, jobOverrides, datafeedOverrides, estimateModelMemory, applyToAllSpaces);
      return response.ok({
        body: result
      });
    } catch (e) {
  ...
  router.get({
    ...
    try {
      ...
      const result = await dataRecognizerJobsExist(client, mlClient, context.core.savedObjects.client, jobSavedObjectService, request, moduleId);
      return response.ok({
        body: result
      });
    } catch (e) {
