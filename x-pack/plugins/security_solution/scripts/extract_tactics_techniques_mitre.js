...
const fetch = require('node-fetch'); // eslint-disable-next-line import/no-extraneous-dependencies
...
const getSubtechniquesOptions = subtechniques => subtechniques.map(t => `{
  label: i18n.translate(
    'xpack.securitySolution.detectionEngine.mitreAttackSubtechniques.${camelCase(t.name)}${t.techniqueId // Seperates subtechniques that have the same name but belong to different techniques
}Description', {
      defaultMessage: '${t.name} (${t.id})'
  }),
  id: '${t.id}',
...
const buildMockThreatData = (tactics, techniques, subtechniques) => {
  ...
  const tactic = tactics.find(tactic => tactic.name === startCase(camelCase(technique.tactics[0])));
  return {
    tactic,
    technique,
    subtechnique
...
async function main() {
  fetch(MITRE_ENTERPRISE_ATTACK_URL).then(res => res.json()).then(json => {
    ...
    const techniques = mitreData.filter(obj => obj.type === 'attack-pattern' && obj.x_mitre_is_subtechnique === false).reduce((acc, item) => {
      let tactics = [];
      const {
        id,
        reference
    ...
    const subtechniques = mitreData.filter(obj => obj.x_mitre_is_subtechnique === true).reduce((acc, item) => {
      let tactics = [];
      const {
        id,
        reference
    ...
    const body = `/*
          ...
          */
          ...
          export const subtechniquesOptions: MitreSubtechniquesOptions[] =
            ${JSON.stringify(getSubtechniquesOptions(subtechniques), null, 2).replace(/}"/g, '}').replace(/"{/g, '{')};
           ...
           */
          export const mockThreatData = ${JSON.stringify(buildMockThreatData(tactics, techniques, subtechniques), null, 2).replace(/}"/g, '}').replace(/"{/g, '{')};
      `;
