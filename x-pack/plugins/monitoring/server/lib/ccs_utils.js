...
function prefixIndexPattern(config, indexPattern, ccs, monitoringIndicesOnly = false) {
  let ccsEnabled = false; // TODO: NP
  // This function is called with both NP config and LP config

  if ((0, _lodash.isFunction)(config.get)) {
    ccsEnabled = config.get('monitoring.ui.ccs.enabled');
  } else {
    ccsEnabled = (0, _lodash.get)(config, 'ui.ccs.enabled');
  }
  ...
  if (ccs === '*') {
    return appendMetricbeatIndex(config, `${prefixedPattern},${indexPattern}`, monitoringIndicesOnly);
  }
