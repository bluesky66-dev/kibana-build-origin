...
exports.queryDatatable = queryDatatable;
/*
...
function queryDatatable(datatable, query) {
  if (query.size) {
    datatable = { ...datatable,
  ...
  if (query.and) {
    query.and.forEach(filter => {
      ...
      if (filter.filterType === 'time') {
        const columnNames = datatable.columns.map(col => col.name); // remove row if no column match
