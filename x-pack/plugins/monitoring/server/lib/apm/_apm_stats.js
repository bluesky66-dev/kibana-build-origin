...
const apmAggFilterPath = ['aggregations.total', 'aggregations.min_events_total.value', 'aggregations.max_events_total.value', 'aggregations.min_mem_rss_total.value', 'aggregations.max_mem_rss_total.value', 'aggregations.max_mem_total_total.value'];
exports.apmAggFilterPath = apmAggFilterPath;

const apmUuidsAgg = maxBucketSize => ({
  total: {
    cardinality: {
...
const apmAggResponseHandler = response => {
  ...
  const memRssMax = (0, _lodash.get)(response, 'aggregations.max_mem_rss_total.value', null);
  const memRssMin = (0, _lodash.get)(response, 'aggregations.min_mem_rss_total.value', null);
  const memTotal = (0, _lodash.get)(response, 'aggregations.max_mem_total_total.value', null);
  return {
    apmTotal,
