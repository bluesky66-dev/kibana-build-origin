...
function defaultCalculation(bucket, key) {
  const value = (0, _lodash.get)(bucket, key, null); // negatives suggest derivatives that have been reset (usually due to restarts that reset the count)
...
async function fetchSeries(req, indexPattern, metric, metricOptions, groupBy, min, max, bucketSize, filters) {
...
function findFirstUsableBucketIndex(buckets, min) {
  ...
  for (let i = 0; i < buckets.length; ++i) {
    const bucketTime = (0, _lodash.get)(buckets, [i, 'key']);
...
function handleSeries(metric, groupBy, min, max, bucketSizeInSeconds, timezone, response) {
  ...
  function getAggregatedData(buckets) {
    ...
    if (metric.debug) {
      console.log(`metric.debug field=${metric.field} bucketsCreated: ${countBuckets((0, _lodash.get)(response, 'aggregations.check'))}`);
  ...
  if (groupBy) {
    return (0, _lodash.get)(response, 'aggregations.groupBy.buckets', []).map(bucket => {
      return {
        groupedBy: bucket.key,
        ...getAggregatedData((0, _lodash.get)(bucket, 'check.buckets', []))
      };
    });
  }

  return getAggregatedData((0, _lodash.get)(response, 'aggregations.check.buckets', []));
}
/**
...
async function getSeries(req, indexPattern, metricName, metricOptions, filters, groupBy, {
  min,
  max,
  bucketSize,
  timezone
}) {
  (0, _error_missing_required.checkParam)(indexPattern, 'indexPattern in details/getSeries');
